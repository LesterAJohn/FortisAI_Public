# Appsmith in FortisAI (macOS)

This document describes how Appsmith is deployed in the FortisAI local development platform on macOS, how it is wired into the shared runtime, and how to use it.
...
Appsmith is started by the mac helper as part of the default local stack.

- Compose file path: `~/fortisai-dev/appsmith/docker-compose.yml`
...
1. `./fortisai-dev-helper.sh setup` writes the Appsmith compose file.
2. `./fortisai-dev-helper.sh up` starts MongoDB and initializes replica set `rs0` before Appsmith startup.
3. `./fortisai-dev-helper.sh down` stops Appsmith with the rest of the local stack.
4. `./fortisai-dev-helper.sh check` validates Appsmith HTTP reachability.
...
```bash
export APPSMITH_HOST_PORT=28080
export APPSMITH_URL=http://localhost:28080
./fortisai-dev-helper.sh up
```
...
```bash
podman ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | grep appsmith
```
...
Then re-check Appsmith with `./fortisai-dev-helper.sh check`.
